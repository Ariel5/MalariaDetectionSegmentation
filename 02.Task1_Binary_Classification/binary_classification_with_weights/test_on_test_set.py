...
def run_test(args):
    ...
    df_thresholds = pandas.read_excel(cwd+'/results/thresholds.xlsx')

    fold_calculations = []
    import tqdm
    # for fold in tqdm.tqdm([0,1,2,3,4]):
    for fold in tqdm.tqdm([3,4]):
        fold_threshold = float(df_thresholds.iloc[fold]['threshold'])
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader_validation):
                ...
                for j in range(len(y_hat)):
                    if y_hat[j][0].item()>fold_threshold:
                        prediction = 1
                    else:
                        prediction = 0
                    result.append({
    ...
    for i in range(len(fold_calculations[0])):
        ...
        if prediction>=len(fold_calculations)/2:
            prediction = 1
        else:
            prediction = 0
        result.append({
    ...
    for i in range(len(list_gt)):
        if list_gt[i]>0:
            den += 1.0
            if list_prediction[i]>0:
                num += 1.0
    performance.append({
        'item':'Sensitivity',
        'Value': num/den,
        'Notes': str(round(num))+ ' / '+str(round(den))
        })
